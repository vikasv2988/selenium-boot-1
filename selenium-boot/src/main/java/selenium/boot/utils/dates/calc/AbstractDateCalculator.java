...
import static selenium.boot.utils.dates.calc.HolidayHandlerType.BACKWARD;
import static selenium.boot.utils.dates.calc.HolidayHandlerType.FORWARD;
import static selenium.boot.utils.dates.calc.HolidayHandlerType.MODIFIED_FOLLOWING;
import static selenium.boot.utils.dates.calc.HolidayHandlerType.MODIFIED_PRECEDING;
...
public abstract class AbstractDateCalculator<E extends Serializable> implements DateCalculator<E>
{
    ...
    private static final Logger log = LoggerFactory.getLogger( AbstractDateCalculator.class );
    ...
    private void checkHolidayValidity( final int businessDays )
    {
        if( businessDays > 0 && holidayHandler != null
                    && ( holidayHandler.getType().equals( BACKWARD )
                                 || holidayHandler.getType().equals( MODIFIED_PRECEDING ) ) )
        {
            throw new IllegalArgumentException( "A " + MODIFIED_PRECEDING + " or " + BACKWARD +
                         ...
                         && ( holidayHandler.getType().equals( FORWARD )
                                      || holidayHandler.getType().equals( MODIFIED_FOLLOWING ) ) )
        {
     ...
     *
     * @throws java.lang.IllegalArgumentException if both calendars have different types of HolidayHandlers or WorkingWeek;
     */
